...
argp.add_argument('--client_args', required=True, type=str, help='Client args.')
argp.add_argument(
    '--records_config_template_path',
    required=True,
    type=str,
...
argp.add_argument(
    '--output_dir', required=True, type=str, help=('Output directory for logs'))
    ...
    for i in range(0, 30):
        ...
        if tcp_connect_subprocess.returncode == 0:
            ...
            dns_resolver_subprocess = subprocess.Popen(
                [
                    os.path.join(_LOCAL_DNS_SERVER_DIR,
                                 'dns_resolver.py'), '--qname',
...
dns_server_invoke_args = os.path.join(os.sep, 'var', 'local',
                                      'local_dns_server', 'dns_server.py')
...
with open(dns_server_subprocess_output, 'w') as l:
    dns_server_subprocess = subprocess.Popen(
        [
            os.path.join(_LOCAL_DNS_SERVER_DIR, 'dns_server.py'), '--port',
            str(53), '--records_config_path', records_config_path
        ],
...
wait_until_dns_server_is_up(dns_server_subprocess, dns_server_subprocess_output)
