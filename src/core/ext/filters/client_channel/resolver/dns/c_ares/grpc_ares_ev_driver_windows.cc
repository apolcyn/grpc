 ...
 * them when readable and write them when writeable. To fit c-ares fd's in to
 * the grpc poller, this GrpcPolledFdWindows class takes advantage of
 * the ares_set_socket_functions API and acts as a virtual socket. It holds
 * its own read and write buffers which are written to and read from c-ares and
 * are used with the grpc windows poller, and it, e.g., manufactures virtual
 * socket error codes when it e.g. needs to do an async read. */
