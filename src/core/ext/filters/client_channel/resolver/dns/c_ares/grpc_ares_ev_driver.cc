...
static grpc_ares_ev_driver* grpc_ares_ev_driver_ref(
    grpc_ares_ev_driver* ev_driver) {
  GRPC_EV_DRIVER_1_1_TRACE_LOG("request:%p Ref ev_driver %p", ev_driver->request,
                       ev_driver);
...
static void grpc_ares_ev_driver_unref(grpc_ares_ev_driver* ev_driver) {
  GRPC_EV_DRIVER_2_TRACE_LOG("request:%p Unref ev_driver %p", ev_driver->request,
                       ev_driver);
  if (gpr_unref(&ev_driver->refs)) {
    GRPC_EV_DRIVER_3_TRACE_LOG("request:%p destroy ev_driver %p", ev_driver->request,
                         ev_driver);
...
static void fd_node_destroy_locked(fd_node* fdn) {
  GRPC_EV_DRIVER_4_TRACE_LOG("request:%p delete fd: %s", fdn->ev_driver->request,
...
grpc_error* grpc_ares_ev_driver_create_locked(grpc_ares_ev_driver** ev_driver,
  ...
  *ev_driver = grpc_core::New<grpc_ares_ev_driver>();
  ...
  GRPC_EV_DRIVER_3_TRACE_LOG("request:%p grpc_ares_ev_driver_create_locked", request);
...
static void on_readable_locked(void* arg, grpc_error* error) {
  ...
  GRPC_EV_DRIVER_2_TRACE_LOG("request:%p readable on |%s| error: %s", fdn->ev_driver->request,
                       fdn->grpc_polled_fd->GetName(), grpc_error_string(error));
...
static void on_writable_locked(void* arg, grpc_error* error) {
  ...
  GRPC_EV_DRIVER_3_TRACE_LOG("request:%p writable on |%s|. error: %s", ev_driver->request,
                       fdn->grpc_polled_fd->GetName(), grpc_error_string(error));
  if (error == GRPC_ERROR_NONE) {
...
static void grpc_ares_notify_on_event_locked(grpc_ares_ev_driver* ev_driver) {
    ...
    for (size_t i = 0; i < ARES_GETSOCK_MAXNUM; i++) {
      if (ARES_GETSOCK_READABLE(socks_bitmask, i) ||
        ...
        if (fdn == nullptr) {
          ...
          GRPC_EV_DRIVER_4_TRACE_LOG("request:%p new fd: %s", ev_driver->request,
                               fdn->grpc_polled_fd->GetName());
        ...
        if (ARES_GETSOCK_READABLE(socks_bitmask, i) &&
          ...
          GRPC_EV_DRIVER_1_3_TRACE_LOG("request:%p notify read on: %s",
                               ev_driver->request,
                               fdn->grpc_polled_fd->GetName());
        ...
        if (ARES_GETSOCK_WRITABLE(socks_bitmask, i) &&
          ...
          GRPC_EV_DRIVER_2_TRACE_LOG("request:%p notify write on: %s",
                               ev_driver->request,
                               fdn->grpc_polled_fd->GetName());
          grpc_ares_ev_driver_ref(ev_driver);
  ...
  if (new_list == nullptr) {
    ev_driver->working = false;
    GRPC_EV_DRIVER_3_TRACE_LOG("request:%p ev driver stop working",
                         ev_driver->request);
  }
}
