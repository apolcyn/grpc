...
static void shutdown_engine(void) {
  ...
  if (grpc_core::Fork::Enabled()) {
    ...
    // gpr_log(GPR_INFO, "apolcyn non-EE shutdown_engine
    // SetResetChildPollingEngineFunc(nullptr)");
...
static bool init_epoll1_linux() {
  ...
  if (grpc_core::Fork::Enabled()) {
    gpr_log(GPR_INFO,
            "apolcyn init_epoll1_linux reset child polling engine func");
    gpr_mu_init(&fork_fd_list_mu);
    gpr_log(
        GPR_INFO,
        "apolcyn non-EE init_epoll1_linux SetResetChildPollingEngineFunc(%p)",
