...
class FakeTcpServer {
 public:
  ...
  enum AcceptMode {
    WAIT_FOR_CLIENT_TO_SEND_FIRST_BYTES,  // useful for emulating ALTS based
                                          // grpc servers
    EAGERLY_SEND_SETTINGS,  // useful for emulating insecure grpc servers (e.g.
  ...
  static void RunServerLoop(FakeTcpServer* self) {
    ...
    while (!gpr_event_get(&self->stop_ev_)) {
      ...
      while (it != peers.end()) {
        ...
        if (self->accept_mode_ == EAGERLY_SEND_SETTINGS) {
...
TEST(AltsConcurrentConnectivityTest,
  ...
  FakeTcpServer fake_backend_server(
      FakeTcpServer::WAIT_FOR_CLIENT_TO_SEND_FIRST_BYTES,
...
TEST(AltsConcurrentConnectivityTest,
  ...
  FakeTcpServer fake_handshake_server(
      FakeTcpServer::EAGERLY_SEND_SETTINGS,
      FakeTcpServer::CloseSocketUponReceivingBytesFromPeer);
