 ...
 public:
  ...
  int Connect(int sockfd, const struct sockaddr *addr,
    ...
    if (addr->sa_family == AF_INET) {
      ...
      inet_ntop(AF_INET, &((sockaddr_in *)addr)->sin_addr, ntop_buf,
                sizeof(ntop_buf));
      gpr_log(GPR_INFO, "str: %s", ntop_buf);
    }
    if (addr->sa_family == AF_INET6) {
      gpr_log(GPR_INFO, "convert AF_INET6 sock to str");
      inet_ntop(AF_INET6, &((sockaddr_in6 *)addr)->sin6_addr, ntop_buf,
                sizeof(ntop_buf));
      gpr_log(GPR_INFO, "str: %s", ntop_buf);
    }
...
void OverrideAddressSortingSocketFactory(
    ...
    std::map<std::string, TestAddress> dest_addr_to_src_addr) {
  address_sorting::SocketFactory *factory = new MockSocketFactory(
      ipv4_supported, ipv6_supported, dest_addr_to_src_addr);
  address_sorting::OverrideSocketFactoryForTesting(factory);
}
