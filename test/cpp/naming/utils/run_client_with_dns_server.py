...
argp.add_argument('--client_args', required=True, type=str,
                  help='Client args.')
argp.add_argument('--client_cwd', required=True, type=str,
                  help='Client cwd.')
argp.add_argument('--records_config_template_basename', required=True, type=str,
...
with open(args.records_config_template_basename, 'r') as template:
...
p = subprocess.Popen(client_invoke_args, env={'GRPC_GO_LOG_VERBOSITY_LEVEL': '3', 'GRPC_GO_LOG_SEVERITY_LEVEL': 'INFO'}, cwd=args.client_cwd)
p.communicate()
sys.stdout.flush()
sys.stderr.flush()
dns_server_subprocess.kill()
